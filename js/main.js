...
$(document).ready(function () {
  ...
  function capNhatTrangThaiHeader() {
    ...
    if (currentUser) {
      ...
      const kyTuDau = currentUser.name
  ...
  if ($("#danh-sach-tim-kiem").length > 0) {
    ...
    thucHienLocPhong();
    $("#chon-duong, #loc-gia-thap, #loc-gia-cao, #sap-xep").on(
      "change keyup",
      thucHienLocPhong,
    );
  ...
  if ($("#ban-do-chinh").length > 0) {
    ...
    function capNhatGhim(danhSach) {
      ...
      danhSach.forEach((phong) => {
        ...
        ghim.bindPopup(
          `<strong>${phong.title}</strong><br>${phong.price.toLocaleString()}đ/tháng`,
        );
  ...
  function thucHienLocPhong() {
    ...
    let loaiChon = $(".loc-loai-phong:checked")
      .map((i, el) => $(el).val())
      .get();
    let tienIchChon = $(".loc-tien-ich:checked")
      .map((i, el) => $(el).val())
      .get();
  ...
  function hienThiDanhSachPhong(ds, container) {
    ...
    ds.forEach((p) => {
      let imgHtml = p.image
        ...
        : '<div class="room-image-placeholder">Ảnh phòng</div>';
      let card = `
  ...
  function napGiaoDienChiTiet(phong) {
    ...
    $("#chiphi-dien").text(
      tienDien > 0 ? `${tienDien.toLocaleString()}đ` : "Không thu",
    );
    $("#chiphi-nuoc").text(
      tienNuoc > 0 ? `${tienNuoc.toLocaleString()}đ` : "Không thu",
    );
    $("#chiphi-wifi").text(
      tienWifi > 0 ? `${tienWifi.toLocaleString()}đ` : "Không thu",
    );
    $("#chiphi-gui-xe").text(
      tienGuiXe > 0 ? `${tienGuiXe.toLocaleString()}đ` : "Không thu",
    );
    $("#chiphi-tong").text(`${Math.round(tong).toLocaleString()}đ`);

    // Tiện ích
    $("#tienich-wifi").text(
      phong.amenities.includes("wifi")
        ? "✔️ Wifi miễn phí"
        : "❌ Không có Wifi",
    );
    $("#tienich-gac").text(
    ...
    $("#lien-he-btn-goi")
      .off("click")
      .on("click", () => alert(`SĐT: ${landlordPhone}`));
    $("#lien-he-btn-nhan")
      .off("click")
      .on("click", () => alert("Hộp thư đang kết nối"));
    $("#lien-he-btn-datlich")
      .off("click")
      .on("click", () => alert("Đã đặt lịch xem phòng"));
